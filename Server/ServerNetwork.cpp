...
vector<int>disconnected_threads;
...
void ServerNetwork::Run() {
    while (1) {
        ...
        td=new thData();
        int id_thread = disconnected_threads.empty()? number_of_threads++: disconnected_threads[0];
        td->idThread=id_thread;
        td->cl=client;
        cout<<"Client #"<<td->idThread<<" connected\n";
        pthread_create(&threads[id_thread],NULL,(THREADFUNCPTR)&Treat,td);
        if(!disconnected_threads.empty())
            disconnected_threads.erase(disconnected_threads.begin());
...
void ServerNetwork::Answer(void *arg){
    ...
    disconnected_threads.push_back(tdL.idThread);
}
...
int ServerNetwork::IsValid(string buffer, string& message,thData& tdL) {
    ...
    if(command=="register"){
        ...
        if(tdL.connected_user=="")
        {
            Login loginCommand(argument_list,server);
            message = loginCommand.Execute();
            if(message.find("Logged in with success.")==0)
                tdL.connected_user=argument_list[0];
        }
        else
            message = "You are already connected.";
        return 1;
    }
    else if(tdL.connected_user==""){
        message = "You must log in first.";
        return 1;
    }
    else if(command=="search"){
        Search searchCommand(argument_list,server,tdL.connected_user);
        ...
        Download downloadCommand(argument_list,server,tdL.connected_user);
        message = downloadCommand.Execute();
        return 1;
    }
    else if(command =="view"){
        if(tdL.last_search_result.empty()){
            message = "Please search books first.";
            return 1;
        }
        View viewCommand(argument_list,server,tdL.last_search_result,tdL.connected_user);
        message = viewCommand.Execute();
        return 1;
    }

    return 0; //in case the command was not found
}
